...
import type { BarData, ClientMsg, DomSnapshot, ReplayState, ServerMsg, StudyUpdate, VpBar } from "@/lib/ws-types"
...
function vpToCandle(vp: VpBar): CandlestickData<Time> {
  ...
  const prices = vp.levels.map((level) => level[0])
...
export function ReplayPrototype() {
  ...
  const applyStudyUpdate = useCallback((ch: string, update: StudyUpdate) => {
    if (ch.startsWith("bars:")) {
      ...
      const updates = [...((update.completions ?? []) as VpBar[]), ...(update.current ? [update.current as VpBar] : [])]
    ...
    } else if (ch.startsWith("dom") && update.current) {
      ...
      if (msg.type === "init") {
        ...
        if (ch.startsWith("bars:")) {
          ...
          if (vpRef.current && seriesRef.current) {
            try {
              seriesRef.current.detachPrimitive(vpRef.current as never)
            } catch {
              // Already detached.
            }
          }
  ...
  return (
    <div className="flex h-svh flex-col bg-background">
      <TopBar
        ...
        showDom={showDom}
        onToggleDom={() => setShowDom((value) => !value)}
      />
      <Chart
